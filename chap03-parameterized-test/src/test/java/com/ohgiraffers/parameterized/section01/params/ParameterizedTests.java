...
class ParameterizedTests {
    ...
    * given when then 패턴에서 사전에 테스트를 이ㅜ해 준비하던 given 부분을 대체할 수 있게된다.
    * */
    ...
    * */
    ...
    void testIsBlank(String input){
        //when
        boolean resutl = StringValidator.isBlank(input);
        //then
        Assertions.assertTrue(resutl);
    }

    //목차 3. 열거형을 이용한 @EnumSource 활용하기
    // 열거형에 작성된 타입들을 파라미터로 전달하여 테스트에 활용할 수 있따.
    @DisplayName("Month에 정의된 타입들이 1~12월 사이의 범위인지 테스트")
    @ParameterizedTest
    @EnumSource(Month.class)
    void testMonthValueIsCollect(Month monthl){

        // when
        boolean result = DataValidator.isCollect(monthl);
